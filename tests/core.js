  ...
  $.Case.new({
    ...
    do() {
      ...
      this.assertEq(p2, foundById2, 'Failed to retrieve instance p2 by ID');
       ...
       this.assertEq(p1, $.Point.$pointInstance1, 'Failed lookup via $.Class.$name');
       this.assertEq(p1, $.Point['$' + p1_id], 'Failed lookup via $.Class.$id');
       this.assertEq(p2, $.Point['$' + p2_id], 'Failed lookup for unnamed instance via $.Class.$id');
    }
  });

  $.Case.new({
    name: 'PrimitiveIntegrationString',
    doc: 'Tests framework integration with native String primitives.',
    do() {
      const testString = "hello";
      const StringPrimitiveClass = _.find('Primitive', 'StringPrimitive');

      this.assert(StringPrimitiveClass, 'Could not find StringPrimitive class definition');

      this.assert(typeof testString.description === 'function', 'String.prototype.description should be added');
      this.assertEq(testString.description(), "hello", 'String description() should return the string itself');

      this.assert(testString.class, 'Native string should have a class() method');
      this.assertEq(testString.class().name(), 'StringPrimitive', 'Native string.class().name() should be StringPrimitive');
    }
  });

   $.Case.new({
    name: 'PrimitiveIntegrationNumber',
    doc: 'Tests framework integration with native Number primitives.',
    do() {
      const testNumber = 123.45;
      const NumberPrimitiveClass = _.find('Primitive', 'NumberPrimitive');

      this.assert(NumberPrimitiveClass, 'Could not find NumberPrimitive class definition');
      this.assert(typeof testNumber.description === 'function', 'Number.prototype.description should be added');
      this.assertEq(testNumber.description(), "123.45", 'Number description() should return string representation');
      this.assertEq(testNumber.class().name(), 'NumberPrimitive', 'Native number.class().name() should be NumberPrimitive');
    }
  });

   $.Case.new({
    name: 'PrimitiveIntegrationArray',
    doc: 'Tests framework integration with native Array primitives.',
    do() {
      const testArray = [1, "two", $.Point.new({x: 3})];
      const ArrayPrimitiveClass = _.find('Primitive', 'ArrayPrimitive');

      this.assert(ArrayPrimitiveClass, 'Could not find ArrayPrimitive class definition');
      this.assert(typeof testArray.description === 'function', 'Array.prototype.description should be added');
      const desc = testArray.description();
      this.assert(desc.startsWith('(') && desc.endsWith(')'), 'Array description format incorrect (parentheses)');
      this.assert(desc.includes('1') && desc.includes('two') && desc.includes('$.Point'), `Array description content incorrect: ${desc}`);

      this.assert(typeof testArray.last === 'function', 'Array.prototype.last should be added');
      this.assertEq(testArray.last(), testArray[2], 'Array last() method failed');
      this.assertEq(testArray.class().name(), 'ArrayPrimitive', 'Native array.class().name() should be ArrayPrimitive');
    }
  });

  $.Case.new({
    name: 'PrimitiveIntegrationBoolean',
    doc: 'Tests framework integration with native Boolean primitives.',
    do() {
      const testBoolTrue = true;
      const testBoolFalse = false;
      const BooleanPrimitiveClass = _.find('Primitive', 'BooleanPrimitive');

      this.assert(BooleanPrimitiveClass, 'Could not find BooleanPrimitive class definition');
      this.assert(typeof testBoolTrue.description === 'function', 'Boolean.prototype.description should be added');
      this.assertEq(testBoolTrue.description(), "true", 'Boolean true description() failed');
      this.assertEq(testBoolFalse.description(), "false", 'Boolean false description() failed');
      this.assertEq(testBoolTrue.class().name(), 'BooleanPrimitive', 'Native boolean.class().name() should be BooleanPrimitive');
    }
  });
}.module({
  name: 'test.core',
